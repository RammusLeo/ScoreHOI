...
from aug_utils import img_processing
...
def vis_results_batch(meta_info, outputs, sample_idx=0, obj_dict_given=None, step=0, init_outputs=None, transform_rot = None):
    # batch_render = {"inputs":{}, "targets":{}, "meta_info":{}}
    save_dir = os.path.join(cfg.vis_dir, f'step_{step}')
    os.makedirs(save_dir, exist_ok=True)
    # outputs_render = {}
    for k,v in outputs.items():
        if torch.is_tensor(v): outputs[k] = v.detach().cpu().numpy()
    if init_outputs is not None:
        for k,v in init_outputs.items():
            if torch.is_tensor(v): init_outputs[k] = v.detach().cpu().numpy()
    aid = meta_info['ann_id'][sample_idx]
    img_path = meta_info['img_path'][sample_idx]
    # shutil.copy(img_path, osp.join(cfg.vis_dir, f'{aid:06d}_img.png'))
    img = cv2.imread(img_path, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION)
    img, img2bb_trans, bb2img_trans, rot, do_flip = img_processing(img, meta_info["bbox"][sample_idx], cfg.MODEL.input_img_shape, "test", augment=False)
    # img = inputs['img'][sample_idx, :3].cpu().numpy().transpose(1,2,0)[:,:,::-1] * 255.0
    # cv2.imwrite(osp.join(save_dir, f'{aid:06d}_img.png'), img)
    white_img = np.ones_like(img) * 255

    smpl_mesh_cam = outputs['smpl_verts'][sample_idx]
    cam_trans = outputs['cam_trans'][sample_idx]
    # cam_trans = batch_render["inputs"]["pred_cam_trans"]
    obj_mesh = obj_dict_given[meta_info['obj_name'][sample_idx]].load_template()
    obj_mesh_cam = obj_dict_given.transform_object(copy.deepcopy(obj_mesh).vertices, outputs['obj_pose'][sample_idx], outputs['obj_trans'][sample_idx])
    smpl_mesh = trimesh.Trimesh(smpl_mesh_cam + cam_trans, smplh.faces, process=False, maintain_order=True)
    obj_mesh_pred = trimesh.Trimesh(obj_mesh_cam + cam_trans, obj_mesh.faces, process=False, maintain_order=True)
    cam_param = {'focal': (cfg.CAMERA.focal[0]*2, cfg.CAMERA.focal[1]*2), 'princpt': cfg.CAMERA.princpt}

    rendered_img = render_mesh(white_img, smpl_mesh, (0.72, 0.72, 0.72), obj_mesh_pred, (0.23, 0.28, 0.82), cam_param=cam_param)  

    if transform_rot is not None:
        smpl_obj_mesh_cam = np.concatenate((smpl_mesh_cam, obj_mesh_cam), axis=0)
        smpl_obj_mesh_cam_rot = np.dot(smpl_obj_mesh_cam, transform_rot)
        smpl_mesh_cam_rot = smpl_obj_mesh_cam_rot[:smpl_mesh_cam.shape[0]]
        obj_mesh_cam_rot = smpl_obj_mesh_cam_rot[smpl_mesh_cam.shape[0]:]

        smpl_mesh_rot = trimesh.Trimesh(smpl_mesh_cam_rot + cam_trans, smplh.faces, process=False, maintain_order=True)
        obj_mesh_pred_rot = trimesh.Trimesh(obj_mesh_cam_rot + cam_trans, obj_mesh.faces, process=False, maintain_order=True)
        cam_param = {'focal': (cfg.CAMERA.focal[0]*2, cfg.CAMERA.focal[1]*2), 'princpt': cfg.CAMERA.princpt}
        rot_render_img = render_mesh(white_img, smpl_mesh_rot, (0.72, 0.72, 0.72), obj_mesh_pred_rot, (0.23, 0.28, 0.82), cam_param=cam_param)  
        
    if init_outputs is not None:
        init_smpl_mesh_cam = init_outputs['smpl_verts'][sample_idx]
        init_cam_trans = init_outputs['cam_trans'][sample_idx]
        # init_smpl_mesh_cam_rot = np.dot(init_smpl_mesh_cam, transform_rot)
        init_obj_mesh_cam = obj_dict_given.transform_object(copy.deepcopy(obj_mesh).vertices, init_outputs['obj_pose'][sample_idx], init_outputs['obj_trans'][sample_idx])
        # init_obj_mesh_cam_rot = np.dot(init_obj_mesh_cam, transform_rot)
        init_smpl_obj_mesh_cam = np.concatenate((init_smpl_mesh_cam, init_obj_mesh_cam), axis=0)
        init_smpl_obj_mesh_cam_rot = np.dot(init_smpl_obj_mesh_cam, transform_rot)
        init_smpl_mesh_cam_rot = init_smpl_obj_mesh_cam_rot[:init_smpl_mesh_cam.shape[0]]
        init_obj_mesh_cam_rot = init_smpl_obj_mesh_cam_rot[init_smpl_mesh_cam.shape[0]:]
        
        init_smpl_mesh = trimesh.Trimesh(init_smpl_mesh_cam + init_cam_trans, smplh.faces, process=False, maintain_order=True)
        init_obj_mesh = trimesh.Trimesh(init_obj_mesh_cam + init_cam_trans, obj_mesh.faces, process=False, maintain_order=True)
        
        init_smpl_mesh_rot = trimesh.Trimesh(init_smpl_mesh_cam_rot + init_cam_trans, smplh.faces, process=False, maintain_order=True)
        init_obj_mesh_rot = trimesh.Trimesh(init_obj_mesh_cam_rot + init_cam_trans, obj_mesh.faces, process=False, maintain_order=True)
     
        init_rendered_img = render_mesh(white_img, init_smpl_mesh, (0.72, 0.72, 0.72), init_obj_mesh, (0.23, 0.28, 0.82), cam_param=cam_param)  
        init_rendered_img_rot = render_mesh(white_img, init_smpl_mesh_rot, (0.72, 0.72, 0.72), init_obj_mesh_rot, (0.23, 0.28, 0.82), cam_param=cam_param)
        
        # cv2.imwrite(osp.join(save_dir, f'{aid:06d}_rendered_init.png'), init_rendered_img)
        wholeimg = np.concatenate((img, init_rendered_img, init_rendered_img_rot, rendered_img, rot_render_img), axis=1)
    else:
        if transform_rot is None:
            wholeimg = np.concatenate((img, rendered_img), axis=1)
        else:
            wholeimg = np.concatenate((img, rendered_img, rot_render_img), axis=1)
    cv2.imwrite(osp.join(save_dir, f'{aid}.png'), wholeimg)

def vis_results_generation(meta_info, outputs, sample_idx=0, obj_dict_given=None, step=0, init_outputs=None, transform_rot = None):
    # batch_render = {"inputs":{}, "targets":{}, "meta_info":{}}
    save_dir = os.path.join(cfg.vis_dir, f'step_{step}')
    os.makedirs(save_dir, exist_ok=True)
    # outputs_render = {}
    ...
    if transform_rot is None:
        return img, rendered_img, None
    else:
        return img, rendered_img, rot_render_img

def vis_results_base(batch, outputs, sample_idx=0, obj_dict_given=None, step=0, init_outputs=None):
    batch_render = {"inputs":{}, "targets":{}, "meta_info":{}}
    save_dir = os.path.join(cfg.vis_dir, f'step_{step}')
    os.makedirs(save_dir, exist_ok=True)
    outputs_render = {}
    for k,v in outputs.items():
        if torch.is_tensor(v): outputs_render[k] = v[sample_idx].detach().cpu().numpy()
        else: outputs_render[k] = v[sample_idx]
    for k,v in batch["inputs"].items():
        if torch.is_tensor(v): batch_render["inputs"][k] = v[sample_idx].detach().cpu().numpy()
        else: batch_render["inputs"][k] = v[sample_idx]
    for k,v in batch["targets"].items():
        if torch.is_tensor(v): batch_render["targets"][k] = v[sample_idx].detach().cpu().numpy()
        else: batch_render["targets"][k] = v[sample_idx]
    for k,v in batch["meta_info"].items():
        if torch.is_tensor(v): batch_render["meta_info"][k] = v[sample_idx].detach().cpu().numpy()
        else: batch_render["meta_info"][k] = v[sample_idx]

    aid = batch_render["meta_info"]['ann_id']
    img_path = batch_render["meta_info"]['img_path']
    # shutil.copy(img_path, osp.join(cfg.vis_dir, f'{aid:06d}_img.png'))
    img = cv2.imread(img_path, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION)
    img, img2bb_trans, bb2img_trans, rot, do_flip = img_processing(img, batch_render["meta_info"]["bbox"], cfg.MODEL.input_img_shape, "test", augment=False)
    # img = inputs['img'][sample_idx, :3].cpu().numpy().transpose(1,2,0)[:,:,::-1] * 255.0
    cv2.imwrite(osp.join(save_dir, f'{aid}_img.png'), img)

    smpl_mesh_cam = outputs_render['smpl_verts']

    cam_trans = outputs_render['camera_translation']
    # cam_trans = batch_render["inputs"]["pred_cam_trans"]
    obj_mesh = obj_dict_given[batch_render["meta_info"]['obj_name']].load_template()
    obj_mesh_cam = obj_dict_given.transform_object(copy.deepcopy(obj_mesh).vertices, outputs_render['obj_pose'], outputs_render['obj_trans'])


    smpl_mesh = trimesh.Trimesh(smpl_mesh_cam + cam_trans, smplh.faces, process=False, maintain_order=True)
    obj_mesh_pred = trimesh.Trimesh(obj_mesh_cam + cam_trans, obj_mesh.faces, process=False, maintain_order=True)
    cam_param = {'focal': (cfg.CAMERA.focal[0], cfg.CAMERA.focal[1]), 'princpt': cfg.CAMERA.princpt}
    rendered_img = render_mesh(img, smpl_mesh, (0.72, 0.72, 0.72), obj_mesh_pred, (0.23, 0.28, 0.82), cam_param=cam_param)  
    cv2.imwrite(osp.join(save_dir, f'{aid}_rendered.png'), rendered_img)
    
    if init_outputs is not None:
        init_smpl_mesh_cam = init_outputs['smpl_verts'][sample_idx]
        init_cam_trans = init_outputs['camera_translation'][sample_idx]
        init_obj_mesh_cam = obj_dict_given.transform_object(copy.deepcopy(obj_mesh).vertices, init_outputs['obj_pose'][sample_idx], init_outputs['obj_trans'][sample_idx])
        init_smpl_mesh = trimesh.Trimesh(init_smpl_mesh_cam + init_cam_trans, smplh.faces, process=False, maintain_order=True)
        init_obj_mesh = trimesh.Trimesh(init_obj_mesh_cam + init_cam_trans, obj_mesh.faces, process=False, maintain_order=True)
        init_rendered_img = render_mesh(img, init_smpl_mesh, (0.72, 0.72, 0.72), init_obj_mesh, (0.23, 0.28, 0.82), cam_param=cam_param)  
        cv2.imwrite(osp.join(save_dir, f'{aid}_rendered_init.png'), init_rendered_img)
