...
def gen_trans_wo_img(bbox, scale, rot, shift, out_shape):
    bb_c_x = float(bbox[0] + 0.5*bbox[2])
    bb_c_y = float(bbox[1] + 0.5*bbox[3])
    bb_width = float(bbox[2])
    bb_height = float(bbox[3])
...
def img_processing(img, bbox, img_shape, data_split, augment=True, force_flip=False, force_scale = -1):
    if augment and data_split == 'train':
...
def smplh_param_processing(human_model_param, cam_param, do_flip, rot, return_trans=False):
    ...
    if do_flip:
        for pair in smplh.orig_flip_pairs:
...
def obj_param_processing(obj_param, cam_param, smpl_center=None, do_flip=False, rot=0):
    pose, trans = obj_param['pose'], obj_param['trans']
    if 'name' in obj_param:
        obj_name = obj_param['name']
    else:
        obj_name = None
    ...
    if not smpl_center is None:
        # move object to the center of smp
