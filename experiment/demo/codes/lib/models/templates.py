...
class SMPLH(object):
    ...
    def get_coords(self, smpl_pose, smpl_shape, smpl_trans = None, gender='neutral'):
        smpl_shape = torch.tensor(smpl_shape.reshape(-1, 10))
        smpl_pose = torch.tensor(smpl_pose.reshape(-1, 156))
        if smpl_trans is None:
            smpl_trans = torch.zeros((smpl_pose.shape[0], 3))
        else:
            smpl_trans = torch.tensor(smpl_trans.reshape(-1, 3))
        output = self.layer[gender](betas=smpl_shape, global_orient=smpl_pose[:, :3], body_pose=smpl_pose[:, 3:66], left_hand_pose=smpl_pose[:, 66:111], right_hand_pose=smpl_pose[:, 111:], transl = smpl_trans)
...
class InteractObject(object):
    def __init__(self, data):
        ...
        if 'feat' in data: self.feat = data['feat']
        else: self.feat = None
        obj = trimesh.load(data['path'], process=False, maintain_order=True, force='mesh')
        self.vertices, self.faces = obj.vertices, obj.faces
        self.vertex_num = len(self.vertices)

    def load_template(self):
        template = trimesh.load(self.path, process=False, maintain_order=True, force='mesh')
        return template
...
class InteractObjectDict(object):
    def __init__(self, use_data="all"):
        if use_data=="all":
            ...
            with open(obj_info_path) as f:
                self.obj_info_behave = json.load(f)
            ...
            print(self.obj_info.keys())
        elif use_data == 'behave':
            obj_info_path = osp.join('data', 'base_data', 'object_models', 'behave', '_info.json')
            with open(obj_info_path) as f:
                self.obj_info = json.load(f)
        elif use_data == 'intercap':
            obj_info_path = osp.join('data', 'base_data', 'object_models', 'intercap', '_info.json')
            with open(obj_info_path) as f:
                self.obj_info = json.load(f)
        else:
    ...
    def get_obj_feat(self, key):
        return getattr(self, key).feat
...
smplh = SMPLH()
obj_dict = InteractObjectDict(use_data="all")