...
from models.resnet_pointnet import PoseResNetObj
...
class Hand4WholeObj(nn.Module):
    def __init__(self, with_obj=True):
        super(Hand4WholeObj, self).__init__()
        self.backbone = PoseResNetObj(50, use_obj_feat=with_obj)
        self.box_net = BoxNet()
    ...
    def forward(self, img, obj_feat):
        batch_size = img.shape[0]
        body_img = F.interpolate(img, cfg.MODEL.input_body_shape)
        img_feat = self.backbone(body_img, obj_feat=obj_feat)
...
class ObjectRegressor(nn.Module):
    def __init__(self):
        ...
        self.obj_feat_out = nn.Linear(64*3+1024+2048, 2048)
        self.relu1 = nn.ReLU()
    ...
    def forward(self, x, obj_feat):
        batch_size = x.shape[0]
        x = x.mean((2,3))
        x = self.obj_feat_out(torch.cat((x, obj_feat),1))
        x = self.relu1(x)
